            ...
            if result:
                ...
                try:
                    ...
                    if _idx_lines:
                        ...
                        try:
                            _mid_changes = []
                            for _start in range(1, 6001, 50):
                                _batch = []
                                for _i in range(_start, min(_start + 50, 6001)):
                                    _code = f'sz{_i:06d}' if _i < 600000 else f'sh{_i - 600000:06d}'
                                    _batch.append(_code)
                                try:
                                    _url = 'http://qt.gtimg.cn/q=' + ','.join(_batch)
                                    _resp = _req.get(_url, timeout=8, headers={'User-Agent': 'Mozilla/5.0'})
